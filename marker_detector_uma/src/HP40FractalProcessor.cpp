...
// Contiene los archivos de calibracion de Huawei P40 Pro
// Implement fractal marker detection and pose estimation.
// Contains calibration files for Huawei P40 Pro.
...
class HP40FractalProcessor : public rclcpp::Node {
...
public:
    HP40FractalProcessor() : Node("HP40FractalProcessor") {
		...
		*/
    	...
    	// Camara elegida, por defecto 0 --> Normal
		// Chosen camera, default 0 --> Normal
		/*
		0 - Normal
		1 - Wide-Angle
		3 - Zoom
		*/
    	...
    	this->get_parameter("camera", camera_);
        ...
        subscription_ = this->create_subscription<std_msgs::msg::UInt8MultiArray>(camera_topic_name, rclcpp::SensorDataQoS(), std::bind(&HP40FractalProcessor::topic_callback, this, std::placeholders::_1));
        

		//Suscripcion al actualizador
		// Subscribe to the updater.
		subscription_update = this->create_subscription<std_msgs::msg::UInt8MultiArray>(update_camera_topic_name, 10, std::bind(&HP40FractalProcessor::update_callback, this, std::placeholders::_1));
		...
		my_cam.readFromXMLFile(myFolderPath + getYMLcalibration(resolution_,camera_));
		...
		RCLCPP_INFO(this->get_logger(), getInfoMessage(resolution_,camera_).c_str());
        
		// Parametros del detector de fractales
		// Parameters of the fractal detector.
    	FDetector.setParams(my_cam,size_); // en [m] ANTES 0.197   0.257
...
private:
    void topic_callback(const std_msgs::msg::UInt8MultiArray & msg ) {
		...
		if (camera_ != 2){ // Con Zoom no se estima la pose // Pose is not estimated with Zoom
			...
			if(FDetector.detect(frame)){ // Detecta el fractal // Detect the fractal.
            	...
            	if(FDetector.poseEstimation()){ // Estima la posicion // Estimate the pose
					...
					std::vector<int> detectedSubMarkers(5,0); // Por defecto vector a 0, ningun submarker detectado
            		...
            		float X = 100*tvec.at<double>(0, 0); //cm
            		float Y = 100*tvec.at<double>(1, 0); //cm
            		float Z = 100*tvec.at<double>(2, 0); //cm
	...
	void update_callback(const std_msgs::msg::UInt8MultiArray & update_msg ) {
		...
		if ((update_msg.data.size() == 4) && (update_msg.data[0] <= 5) && (update_msg.data[1] <= 2) && (update_msg.data[2] <= 1) && (update_msg.data[3] == 111)) {
			
			// Mensaje valido
			// Valid message.

			if (update_msg.data[1] == 2){

				// Modo zoom activado en el movil
				// Zoom activated
				camera_ = 2;
				RCLCPP_INFO(this->get_logger(), "Zoom mode enabled. Image is not being processed.");

			}
			else{

				resolution_ = update_msg.data[0];
				camera_ = update_msg.data[1];
	...
	std::string getInfoMessage(int resolution, int camera){

		std::string info_msg = "Huawei P40 - ";
	...
	std::string getYMLcalibration(int resolution, int camera){

		std::string calibration_file_name = "/huaweip40_";
		
		// Camara empleada
		// Used camera

		if (camera == 1){
			calibration_file_name += "W_";
		} 
		else{
			calibration_file_name += "N_";
		}
		...
		if (resolution == 1){
			...
			calibration_file_name += "1440x720.yml";
		}
		else if (resolution == 3){
			...
			calibration_file_name += "720x720.yml";
		}
...
int main(int argc, char *argv[]) {
    ...
    rclcpp::spin(std::make_shared<HP40FractalProcessor>());
    rclcpp::shutdown();
    return 0;
}
