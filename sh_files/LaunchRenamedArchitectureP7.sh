...
terminator -e "source /opt/ros/humble/setup.bash;
source ros2_ws/install/local_setup.bash;
echo 'Launching ${AgentName}_GP7FractalProcessor node with Agent name: $AgentName';
ros2 run fractal_pkg GP7FractalProcessor --ros-args -p AgentName:=$AgentName -p resolution:=$Resolution -p size:=$Size --remap __node:=${AgentName}_GP7FractalProcessor;
exec bash"
