...
terminator -e "source /opt/ros/humble/setup.bash;
source ros2_ws/install/local_setup.bash;
echo 'Launching GP7FractalProcessor node with Agent name: $AgentName';
ros2 run marker_detector_uma GP7FractalProcessor --ros-args -p AgentName:=$AgentName -p resolution:=$Resolution -p size:=$Size;
exec bash"
